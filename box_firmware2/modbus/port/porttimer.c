 ...
 * Copyright (C) 2016 Kutkov Oleg <kutkov.o@yandex.ru>
 *
...
/* ----------------------- Defines ------------------------------------------*/
#define MB_TIMER_PRESCALER      ( 1024UL ) // 1024UL
#define MB_TIMER_TICKS          ( F_CPU / MB_TIMER_PRESCALER )
#define MB_50US_TICKS           ( 60000UL )

/* ----------------------- Static variables ---------------------------------*/
static USHORT   usTimerOCRADelta;

/* ----------------------- Start implementation -----------------------------*/
void xMBPortTimersInit( USHORT usTim1Timerout50us )
{
...
vMBPortTimersEnable(  )
{
    ...
    TCCR1B |=  _BV( CS12 ) | _BV( CS10 ); // _BV( CS12 ) ; // | _BV( CS10 );
}
